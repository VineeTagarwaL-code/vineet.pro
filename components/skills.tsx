...
import { Express } from "@/assests/logos/express"
...
<SectionWrapper>
    <div className="mt-8 flex justify-center items-center flex-col">
        ...
        <h1 className="font-jetbrain text-center text-5xl mb-16"><span className=" text-green-300">code</span><span className="text-foreground">:</span><span className=" text-cyan-300">skills</span></h1>
        </Tip>
        <div
        
        className="flex flex-nowrap flex-col gap-8 justify-center items-center ">
            <div className="flex-wrap flex gap-8 justify-center items-center">
                {languages.map((skill:skillType) => {
                    return (
                        <SkillCard key={skill.name } skill={skill.name} logo={skill.logo} contClass="shadow-[6px_6px_0px_1px_#2da4b9] min-w-[150px] md:min-w-[180px]"/>
                    )
                })}
            </div>
            ...
            <div className="flex-wrap flex gap-6 justify-center items-center">
                {frameworks.map((skill:skillType) => {
                    return (
                        <SkillCard key={skill.name } skill={skill.name} logo={skill.logo} contClass="shadow-[6px_6px_0px_1px_#34c07c] min-w-[150px] md:min-w-[180px]"/>
                    )
                })}
            </div>
            <div className="flex-wrap flex gap-6 justify-center items-center">
                {databases.map((skill:skillType) => {
                    return (
                        <SkillCard key={skill.name } skill={skill.name} logo={skill.logo} contClass="shadow-[6px_6px_0px_1px_#dcab70] max-w-[150px] min-w-[150px] "/>
                    )
                })}
            </div>
            <div className="flex-wrap flex gap-6">
                {tools.map((skill:skillType) => {
                    return (
                        <SkillCard key={skill.name } skill={skill.name} logo={skill.logo} contClass="shadow-[6px_6px_0px_1px_#dc8070] max-w-[150px] min-w-[150px] "/>
                    )
                })}
            </div>
        </div>
